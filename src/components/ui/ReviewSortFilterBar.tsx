...
export function ReviewSortFilterBar({ filters, onChange }: ReviewSortFilterBarProps) {
  ...
  useEffect(() => {
    if (waitingForAction === 'REVIEW_SEARCH' && filters.stars === '5' && filters.search) {
      dispatchTutorialAction('REVIEW_SEARCH')
    }
  }, [filters.stars, filters.search, waitingForAction, dispatchTutorialAction])
